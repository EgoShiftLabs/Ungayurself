...
let achievements = {
  ...
  'video-watcher': { title: 'Transformation Witness', description: 'Watched the transformation video', icon: '🎬', unlocked: false }
};
let progress = {
  memeClicks: 0,
  socialClicks: 0,
...
function trackVideoWatch() {
  if (!progress.hasWatchedVideo) {
    progress.hasWatchedVideo = true;
...
function setupVideoTracking() {
  ...
  video.addEventListener('timeupdate', () => {
    // Track if user watches at least 10 seconds
    if (video.currentTime > 10 && !progress.hasWatchedVideo) {
      trackVideoWatch();
    }
  });
}
